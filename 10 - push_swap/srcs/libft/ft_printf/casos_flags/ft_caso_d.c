...
/*   Updated: 2021/04/20 16:51:35 by nayran           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_caso_d(int d, t_flags flags)
{
	...
	if (d < 0 && flags.zero != 0)
	{
		if ((flags.ponto >= flags.comprimento
				|| (int)ft_strlen(s) >= flags.comprimento)
			|| (flags.comprimento > (int)ft_strlen(s)
				&& flags.zero == 1 && flags.bool_ponto == 0))
		{
			flags.tipo = -1;
			d *= -1;
			count++;
